...
#define GLOBAL_MIN_THREAD 			(4)
#define GLOBAL_MAX_THREAD 			(128)
#define GLOBAL_IDLE_SECOND 			(60)

thr_pool_t *global_thpool;

int init_global_thpool(void)
{
    global_thpool = thr_pool_create(GLOBAL_MIN_THREAD, GLOBAL_MAX_THREAD,
            GLOBAL_IDLE_SECOND, NULL);
    if(!global_thpool)
        return -EINVAL;
...
thr_pool_t *get_global_thpool(void)
{
    return global_thpool;
}
